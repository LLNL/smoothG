...
void HybridSolver::AssembleTwoPhaseHelper(
    const mfem::Vector& elem_scaling_inverse)
{
    two_phase_core_.resize(Minv_.size());
    mfem::Array<int> col_marker(mgL_.GetD().NumCols());
    col_marker = -1;

    mfem::DenseMatrix AinvDMinv, MinvDT;
    mfem::Array<int> local_vertexdof, local_edgedof;

    for (int iAgg = 0; iAgg < nAggs_; ++iAgg)
    {
        AinvDMinv = smoothg::Mult(Ainv_[iAgg], DMinv_[iAgg]);
        MinvDT.Transpose(DMinv_[iAgg]);
        two_phase_core_[iAgg] = smoothg::Mult(MinvDT, AinvDMinv);
        two_phase_core_[iAgg] -= Minv_ref_[iAgg];
        two_phase_core_[iAgg] *= (elem_scaling_inverse[iAgg] * -1.0);
    }
}

void HybridSolver::AssembleHybridTwoPhase(
        const mfem::Vector& elem_scaling_inverse,
        const std::vector<mfem::DenseMatrix>& dMdS,
        const std::vector<mfem::DenseMatrix>& dTdsigma,
...
{
    AssembleTwoPhaseHelper(elem_scaling_inverse);

    const auto& Agg_vertexdof = mgL_.GetGraphSpace().VertexToVDof();

    mfem::SparseMatrix* A00 = new mfem::SparseMatrix(num_multiplier_dofs_);
    mfem::SparseMatrix* A11_tmp = new mfem::SparseMatrix(mgL_.NumVDofs());
    mfem::SparseMatrix* A01 = new mfem::SparseMatrix(num_multiplier_dofs_, mgL_.NumVDofs());
    mfem::SparseMatrix* A10 = new mfem::SparseMatrix(mgL_.NumVDofs(), num_multiplier_dofs_);
    ...
    mfem::SparseMatrix* A11 = mfem::Add(1.0, *A11_tmp, -1.0, dTdS);

    auto ToParMatrix = [this](mfem::SparseMatrix* A)
    {
        mfem::Array<int> row_starts, col_starts;
        GenerateOffsets(comm_, A->NumRows(), row_starts);
        GenerateOffsets(comm_, A->NumCols(), col_starts);

        auto pA = new mfem::HypreParMatrix(comm_, A->NumRows(), A->NumCols(),
    ...
    auto tmp = ParMult(*multiplier_td_d_, *A00, multiplier_start_);
    mfem::HypreParMatrix* pA00(mfem::ParMult(tmp.get(), multiplier_d_td_.get()));
...
//    pA00 = ToParMatrix(A00);
    auto pA01_tmp = ToParMatrix(A01);
    auto pA10_tmp = ToParMatrix(A10);
    auto pA11 = ToParMatrix(A11);

    auto pA01 = mfem::ParMult(&pScale, pA01_tmp);
    auto pA10 = mfem::ParMult(pA10_tmp, &pScale);

    delete pA01_tmp;
    delete pA10_tmp;
    delete A11_tmp;

    op.SetBlock(0, 0, pA00);
    op.SetBlock(0, 1, pA01);
    op.SetBlock(1, 0, pA10);
    op.SetBlock(1, 1, pA11);
    op.owns_blocks = true;
