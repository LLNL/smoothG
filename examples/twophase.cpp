...
class TwoPhaseSolver
{
    ...
    std::vector<double> step_average_coarsest_linear_iter_;
    std::vector<double> step_average_fine_linear_iter_;
    std::vector<double> step_average_mid_linear_iter_;
    std::vector<double> step_time_;
    std::vector<double> cumulative_step_time_;
    std::vector<double> step_CFL_const_;
    std::vector<std::vector<double>> step_level_time_;
...
//        int num_edges_total = num_edges_res + num_injectors + num_producers;
        ...
        if (!file.is_open())
        {
            std::cerr << "Error in opening file " << c2f_filename << "\n";
...
FindWellCells(const Graph& graph, int num_injectors, int num_producers, int well_height)
{
    ...
    {
        std::vector<int> injector_cells;
        for (int j = 0; j < vert_vert.RowSize(i); ++j)
        {
            int i_friend = vert_vert.GetRowColumns(i)[j];
            if (i_friend == i) { continue; }
            injector_cells.push_back(i_friend);

            std::cout<<"injector_cell: "<<i_friend<<"\n";
        }
        out.push_back(injector_cells);
    }

    for (int w = 0; w < num_producers; ++w)
    {
        std::vector<int> producer_cells;
        int offset = edge_vert.NumRows()-(num_producers-w)*well_height;
        for (int i = offset; i < offset + well_height; ++i)
        {
            assert(edge_vert.RowSize(i) == 1);
            producer_cells.push_back(edge_vert.GetRowColumns(i)[0]);
        }
        out.push_back(producer_cells);
    }
...
mfem::Array<int> *
AddWellCellAgg(const mfem::Array<int>& partition,
               const std::vector<std::vector<int>>& well_cells)
{
    auto out = new mfem::Array<int>();
    partition.Copy(*out);

    const int num_parts = partition.Max()+1;

    mfem::SparseMatrix agg_vert = PartitionToMatrix(partition, num_parts);
    agg_vert.SortColumnIndices();

    mfem::Array<int> all_aggs(num_parts);
    std::iota(all_aggs.begin(), all_aggs.end(), 0);
    int new_agg_id = num_parts;

    for (std::vector<int> cells : well_cells)
    {
        mfem::Array<int> cells_arr(cells.data(), cells.size());
        cells_arr.Sort();
        std::cout << "isolating well cells:\n";
        cells_arr.Print(std::cout, 8);
        auto agg_wellcell = ExtractRowAndColumns(agg_vert, all_aggs, cells_arr);

        for (int r = 0; r < agg_wellcell.NumRows(); ++r)
        {
            if (agg_wellcell.RowSize(r) == 0) { continue; }
            for (int j = 0; j < agg_wellcell.RowSize(r); ++j)
            {
                int cell = cells_arr[agg_wellcell.GetRowColumns(r)[j]];
                (*out)[cell] = new_agg_id;
            }
            new_agg_id++;
        }
...
int main(int argc, char* argv[])
{
    ...
    const int max_iter = upscale_param.max_levels > 1 ? 100 : 200;
...
//    fas_param.coarse.max_num_iter = use_vcycle ? 10 : max_iter;
    fas_param.fine.max_num_iter = use_vcycle ? 1 : 1;
    fas_param.mid.max_num_iter = use_vcycle ? 1 : 1;
    fas_param.coarse.max_num_iter = use_vcycle ? 20 : 10;
    fas_param.coarse.print_level = use_vcycle ? print_level : print_level;
    fas_param.fine.print_level = use_vcycle ? print_level : print_level;
    fas_param.mid.print_level = use_vcycle ? print_level : print_level;
...
//    fas_param.nl_solve.max_num_iter = use_vcycle ? max_iter : 1;
    fas_param.nl_solve.print_level = use_vcycle ? 1 : 1;
    fas_param.nl_solve.max_num_iter = use_vcycle ? max_iter : max_iter;
    ...
    {
        //    std::string path = "spe10_bottom_layer_2d";
        bool need_getline = true;
        int num_vert_res = 13200;
        int nnz_res = num_vert_res * 4;
        int num_edges_res = 26680;

        if (path == "spe10_bottom_layer_3d_no_inactive")
        {
            //    std::string path = "spe10_bottom_layer_3d_constrast_10^5/";
            //    std::string path = "spe10_bottom_layer_3d_constrast_10^3";
            need_getline = false;
            num_vert_res = 12321;
        ...
        else if (path == "spe10_top_layer_3d" ||
                 path == "spe10_bottom_layer_3d")
        {
            //    std::string path = "spe10_bottom_layer_3d";
            num_vert_res = 13200;
            nnz_res = num_vert_res * 6;
            num_edges_res = 53080;
            inject_rate *= 0.6096;
...
//        else if (path == "egg")
        else if (path == "egg_single_perforation")
        {
//            num_vert_res = 18553;
//            nnz_res = num_vert_res * 6;
//            num_edges_res = 59205;
...
//            num_edges_res = 1534707;
        ...
        else if (path == "egg_well_model_2x2x2")
        {
            num_vert_res = 148424;
            nnz_res = num_vert_res * 6;
            num_edges_res = 459456;
        ...
        else if (path == "egg_well_model_3x3x3")
        {
            num_vert_res = 500931;
            nnz_res = num_vert_res * 6;
            ...
            ess_attr.SetSize(3, 1);
            problem_for_plot.reset(new EggModel("refined_egg_3x3x3.vtk", 0, 0, ess_attr));
        }
        else if (path == "norne")
        {
            num_vert_res = 44915;
        ...
        else if ((path == "saigup") || (path == "saigup_well_model"))
        {
            num_vert_res = 78720;
            nnz_res = 505918;
        ...
        else if ((path == "refined_saigup") || (path == "saigup_well_model_2x2x2"))
        {
            num_vert_res = 629760;
            nnz_res = 3913208;
        ...
        else if (path != "spe10_bottom_layer_2d")
        {
    ...
    bool special_part = false;//(read_from_file == false || isolate_well_cells);
    unique_ptr<mfem::Array<int>> partition(special_part ? new mfem::Array<int> : nullptr);

    if (special_part && upscale_param.max_levels > 1)
    {
...
//            SetAttrForAggPrint(*problem_ptr, *partition);
    ...
    if (false)
    {
        auto PsigmaT = smoothg::Transpose(hierarchy.GetPsigma(0));
        //    PsigmaT.Print();

        mfem::Array<int> coarse_well_flux_dofs;
        for (int i = 0; i < PsigmaT.NumRows(); ++i)
        {
            int* cols = PsigmaT.GetRowColumns(i);
            for (int j = 0; j < PsigmaT.RowSize(i); ++j)
            {
                if (cols[j] >= 59205)
                {
                    assert(cols[j] < 59289);
                    coarse_well_flux_dofs.Append(i);
                    break;
                }
            }
        }
        //    coarse_well_flux_dofs.Print();

        mfem::Array<int> dof_part_fine(59289+18561+18561);
        problem->BlockOffsets().Print();
        for(int b = 0; b < problem->BlockOffsets().Size()-1; ++b)
        {
            for (int i = problem->BlockOffsets()[b]; i < problem->BlockOffsets()[b+1]; ++i)
            {
                dof_part_fine[i] = 2*b;
            }
        }

        for (int i = problem->BlockOffsets()[1]-7*12; i < problem->BlockOffsets()[1]; ++i)
        {
            dof_part_fine[i] = 1;
        }

        for (int i = problem->BlockOffsets()[2]-num_injectors; i < problem->BlockOffsets()[2]; ++i)
        {
            dof_part_fine[i] = 3;
        }

        std::cout<<"dof_part_fine size = "<<dof_part_fine.Size()<<"\n";
        std::ofstream dof_part_fine_file("mixed_monolithic_dof_part_fine.txt");
        dof_part_fine.Print(dof_part_fine_file, 1);


        auto& agg_face = hierarchy.GetGraph(1).VertexToEdge();
        mfem::Array<int> dof_part_coarse(agg_face.NumCols()+2*agg_face.NumRows());

        for (int i = hierarchy.BlockOffsets(1)[0]; i < hierarchy.BlockOffsets(1)[1]; ++i)
        {
            dof_part_coarse[i] = 0;
        }
        for (int dof : coarse_well_flux_dofs)
        {
            dof_part_coarse[dof] = 1;
        }

        for (int i = hierarchy.BlockOffsets(1)[1]; i < hierarchy.BlockOffsets(1)[2]; ++i)
        {
            dof_part_coarse[i] = 2;
        }

        for (int i = hierarchy.BlockOffsets(1)[2]-num_injectors; i < hierarchy.BlockOffsets(1)[2]; ++i)
        {
            dof_part_coarse[i] = 3;
        }

        for (int i = hierarchy.BlockOffsets(1)[2]; i < hierarchy.BlockOffsets(1)[2]+agg_face.NumRows(); ++i)
        {
            dof_part_coarse[i] = 4;
        }

        std::cout<<"dof_part_coarse size = "<<dof_part_coarse.Size()<<"\n";
        std::ofstream dof_part_coarse_file("mixed_monolithic_dof_part_coarse.txt");
//        mfem::Array<int> dof_part_coarse2(4);
//        dof_part_coarse2 = 8;
...
//    for (int i = 0; i < num_injectors; ++i)
//    {
//        int agg = agg_agg.NumRows() - num_injectors + i;
//        std::cout << "aggs connected to injector #" << i << " (agg "<<agg<<"):\n";

//        for (int j = 0; j < agg_agg.RowSize(agg); ++j)
//        {
//            if (agg_agg.GetRowColumns(agg)[j] != agg)
//            {
//                std::cout << agg_agg.GetRowColumns(agg)[j] << " ";
//            }
//        }
//        std::cout << "\n";
//    }
...
//        std::ifstream flux_file("spe10_1to5_flux_basis_357.vtk");
        std::string b_id = std::to_string(100);
        std::ifstream flux_file("egg_lvl2_flux_basis_"+b_id+".txt");
        mfem::GridFunction flux_basis(problem_for_plot->GetSigmaSpace());

        auto& Psigma = hierarchy.GetPsigma(0);
        mfem::Vector flux_tmp(Psigma.NumCols()), flux_fine(Psigma.NumRows());
        flux_tmp = 0.0;
        flux_tmp.Load(flux_file, flux_tmp.Size());
        flux_tmp.SetSize(Psigma.NumCols());
        Psigma.Mult(flux_tmp, flux_fine);
        ...
        std::cout<<"flux_basis size = "<<flux_basis.Size()<<"\n";
//        std::ofstream flux_outfile("spe10_1to5_flux_basis_357_ref1.vtk");
        std::ofstream flux_outfile("egg_lvl2_flux_basis_"+b_id+"_ref1.vtk");
        flux_basis.SaveVTK(flux_outfile, "lvl2_flux_basis_"+b_id+"", 1);

        std::ifstream pot_file("egg_lvl2_potential_"+b_id+".txt");
        mfem::GridFunction pot_basis(problem_for_plot->GetUSpace());
        auto& Pu = hierarchy.GetPu(0);
        mfem::Vector pot_tmp(Pu.NumCols()), pot_fine(Pu.NumRows());;
        pot_tmp = 0.0;
        pot_tmp.Load(pot_file, pot_tmp.Size()-8);
        pot_tmp.SetSize(Pu.NumCols());
        Pu.Mult(pot_tmp, pot_fine);
        for (int ii = 0; ii <pot_basis.Size(); ++ii)
        {
            pot_basis[ii] = pot_fine[ii];
        }

        std::ofstream pot_outfile("egg_lvl2_potential_"+b_id+"_ref1.vtk");
...
//            problem_ptr->VisSetup(soutf, flux_s[l], 0.0, 0.0, "Flux, "+base_msg, false, false, partition->GetData());
            ...
            {
                problem_ptr->VisSetup(soutp, pres_s[l], 0.0, 0.0, "Pressure, "+base_msg, false, true);
                problem_ptr->VisSetup(souts, Ss[l], Mobility::S_min_, Mobility::S_max_,
                                      "Saturation, "+base_msg, false, true);
            }
        }
...
void SetOptions(FASParameters& param, bool use_vcycle, int num_backtrack, double diff_tol)
{
    ...
    param.coarse_correct_tol = use_vcycle ? 1e-2 : 1e-6;
...
TwoPhaseSolver::TwoPhaseSolver(const DarcyProblem& problem, Hierarchy& hierarchy,
    ...
    : level_(level), time_stepping_param_(time_stepping_param), solver_param_(solver_param),
      ...
      step_converged_(true), step_level_time_(hierarchy.NumLevels()),
...
//        PrintTable(step_CFL_const_, "CFL constants");
        ...
        {
            PrintTable(step_level_time_[ii], "level "+std::to_string(ii)+" time ");
        }
        PrintTable(step_CFL_const_, "CFL constants");
...
//        PrintForLatexTable(step_num_backtrack_, "# backtrack");
        ...
        for (int ii = 0; ii < step_level_time_.size(); ++ii)
        {
            std::string new_col = ",level_"+std::to_string(ii)+"_time";
            step_all_file << new_col;
        }
        step_all_file << "\n";

        for (int ii = 0; ii < step_nonlinear_iter_.size(); ++ii)
        {
            double step_dt = ii ? cumulative_time[ii]-cumulative_time[ii-1]
                                : cumulative_time[ii];
            step_all_file<<cumulative_time[ii]<<", "<<step_dt<<", "<<step_CFL_const_[ii]
                         <<", "<<step_time_[ii]<<", "<<step_nonlinear_iter_[ii]<<", "
                         <<step_average_fine_linear_iter_[ii]<<", "
                         <<step_average_coarsest_linear_iter_[ii];
            for (int iii = 0; iii < step_level_time_.size(); ++iii)
            {
                step_all_file <<", "<< step_level_time_[iii][ii];
            }
            step_all_file << "\n";
        }
...
//        rhs[rhs.Size()-1] = Mobility::S_max_;
...
//        int num_coarse_lin_iter = solver.GetLevelSolver(hierarchy_->NumLevels()-1).GetNumLinearIterations();
        auto num_avg_fine_lin_iter = solver.GetLevelSolver(0).GetNumLinearIterationsMean();
//        int num_mid_lin_iter = hierarchy_->NumLevels() > 1 ? solver.GetLevelSolver(1).GetNumLinearIterations() : 0;
        int c_lvl = hierarchy_->NumLevels() - 1;
        auto num_avg_coarse_lin_iter = solver.GetLevelSolver(c_lvl).GetNumLinearIterationsMean();
        step_average_coarsest_linear_iter_.push_back(round(num_avg_coarse_lin_iter*100)/100);
...
////                    num_mid_lin_iter / ((double)solver.GetNumCoarsestIterations());
        step_average_fine_linear_iter_.push_back(round(num_avg_fine_lin_iter*100)/100);
        for (int l = 0; l < level_time_.size(); ++l)
        {
            step_level_time_[l].push_back(solver.GetLevelTime(l));
            level_time_[l] += solver.GetLevelTime(l);
        }
      ...
      scales_(3), D_fine_(hierarchy_.GetMatrix(0).GetD())
{
    ...
    *DT_ *= (1. / dt_ / density_);
    ...
    mfem::SparseMatrix Ds_proc(hierarchy.GetMatrix(level).GetDs());

    const mfem::Array<int>& ess_dofs_f = ess_dofs_;
    if (ess_dofs_f.Size()) { Ds_proc.EliminateCols(ess_dofs_f); }
    auto& edof_trueedof = hierarchy.GetMatrix(level).GetGraphSpace().EDofToTrueEDof();
...
//        for (int i = 0; i < 3; ++i )
//        {
//            std::cout<<"resid block" << i << ": " << out.GetBlock(i).Norml2() << "\n";
//        }
//    }
...
//    GetDiag(*dTdS) += Ms_;
    ...
    mfem::BlockVector dx2(true_blk_dx);
    dx2 = 0.0;
...
//            auto local_dTdsigma = Build_dTdsigma(space, darcy_system_.GetD(), U_FS);

//            auto& vert_edof = space.VertexToEDof();
//            mfem::Array<int> local_edofs, local_vdofs(1);

//            const int S_size = darcy_system_.GetPWConstProj().NumCols();
//            mfem::SparseMatrix dTdsigma2(S_size, darcy_system_.NumEDofs());
//            for (int i = 0; i < vert_edof.NumRows(); ++i)
//            {
//                GetTableRow(vert_edof, i, local_edofs);
//                local_vdofs = i;
//                dTdsigma2.AddSubMatrix(local_vdofs, local_edofs, local_dTdsigma[i]);
//            }
//            dTdsigma2.Finalize();


//            dTdsigma2 *= (dt_ * density_);
//            mfem::Vector rand_vec(dTdsigma2.NumCols());
//            rand_vec.Randomize(myid_);

//            mfem::Vector dTdsig_rand1(dTdsigma2.NumRows());
//            mfem::Vector dTdsig_rand2(dTdsigma2.NumRows());
//            dTdsig_rand1 = 0.0;
//            dTdsig_rand2 = 0.0;

//            dTdsigma->Mult(rand_vec, dTdsig_rand1);
//            dTdsigma2.Mult(rand_vec, dTdsig_rand2);

//            auto Check = [&](mfem::Vector & v, mfem::Vector & u, std::string op)
//            {
//                v -= u;
//                double diff = mfem::ParNormlp(v, 2, comm_) / mfem::ParNormlp(u, 2, comm_);
//                if (diff > 1e-10 && myid_ == 0)
//                {
//                    std::cerr << "\nWarning: || " << op << " || = " << diff << " !!!\n";
//                }
//            };
//            Check(dTdsig_rand1, dTdsig_rand2, "dTdsig1 - dTdsig2");
...
//    dx2 -= blk_dx;
////    dx2.GetBlock(1)[dx2.BlockSize(1)-1] = 0.0;

////    dx2.GetBlock(0).Print();
//    for (int i = 0; i < 3; ++i )
//    {
//        std::cout << "x diff block" << i << ": "
//                  << dx2.GetBlock(i).Norml2() / blk_dx.GetBlock(i).Norml2() << "\n";
//    }

//    mfem::socketstream vis;
//    problem_ptr->VisSetup(vis, dx2.GetBlock(1), 0.0, 0.0, "hybrid_sol");
//    mfem::socketstream vis2;
//    problem_ptr->VisSetup(vis2, blk_dx.GetBlock(1), 0.0, 0.0, "primal_sol");
                               ...
                               mfem::BlockVector& true_dx)
{
    ...
    unique_ptr<mfem::Solver> schur11_inv(BoomerAMG(*schur(1, 1)));
...
//    mfem::Array<int> new_offsets(6);
//    new_offsets[0] = 0;
//    new_offsets[3] = true_resid.GetBlock(1).Size() - 8;
//    new_offsets[4] = 8;
//    new_offsets[5] = true_resid.GetBlock(2).Size();

//    if (prec.NumRows() > 10000)
//    {
//        int err = hypre_ParCSRMatrixPrint(*par_mono_op, "mixed_monolithic_fine_A");
//        hypre_ParCSRMatrix* test = hypre_ParCSRMatrixRead(comm_, "mixed_monolithic_fine_A");
//        assert(err == 0);
////        par_mono_op.reset(new mfem::HypreParMatrix(test));
//        mfem::HypreParVector par_resid(comm_, par_mono_op->N(), true_resid.GetData(), par_mono_op->RowPart());
//        err = hypre_ParVectorPrint(par_resid, "mixed_monolithic_fine_rhs");
//        hypre_ParVector* test_rhs = hypre_ParVectorRead(comm_, "mixed_monolithic_fine_rhs");
//        assert(err == 0);


//        new_offsets[1] = true_blk_offsets_[1] - 7*12;
//        new_offsets[2] = 7*12;
//        new_offsets.PartialSum();

//        std::ofstream offset_file("mixed_monolithic_fine_offsets");
//        new_offsets.Print(offset_file, 1);
//    }
//    else
//    {
//        int err = hypre_ParCSRMatrixPrint(*par_mono_op, "mixed_monolithic_coarse_A");
//        hypre_ParCSRMatrix* test = hypre_ParCSRMatrixRead(comm_, "mixed_monolithic_coarse_A");
//        assert(err == 0);
////        par_mono_op.reset(new mfem::HypreParMatrix(test));
//        mfem::HypreParVector par_resid(comm_, par_mono_op->N(), true_resid.GetData(), par_mono_op->RowPart());
//        err = hypre_ParVectorPrint(par_resid, "mixed_monolithic_coarse_rhs");
//        hypre_ParVector* test_rhs = hypre_ParVectorRead(comm_, "mixed_monolithic_coarse_rhs");
//        assert(err == 0);

//        new_offsets[1] = true_blk_offsets_[1] - 7*12;
//        new_offsets[2] = 7*12;
//        new_offsets.PartialSum();
//        std::ofstream offset_file("mixed_monolithic_coarse_offsets");
//        new_offsets.Print(offset_file, 1);
//    }
    ...
    if (false)
    {
        mfem::Vector sol_debug(true_dx);
        mfem::UMFPackSolver solver_debug(*mono_op);
        solver_debug.Mult(true_resid, sol_debug);
        sol_debug -= true_dx;
        std::cout <<"mixed sol diff = "<<sol_debug.Norml2() / true_dx.Norml2() << "\n";
    }
...
//    auto local_dTdsigma = Build_dTdsigma(space, D, flux, Mobility::FractionalFlow(S));
//    auto local_dTdsigma = Build_dTdsigma(space, micro_upwind_flux_, D, flux, Mobility::FractionalFlow(S));
    auto local_dTdsigma = Build_dTdsigma(space, D, U_FS);

//    TwoPhaseHybrid solver(darcy_system_, &ess_attr_);
    TwoPhaseHybrid& solver = hierarchy_.GetTwoPhaseHybridSolver(level_);
                                ...
                                mfem::BlockVector& true_dx)
{
    ...
    mfem::BlockVector primal_dx(true_dx.GetBlock(1).GetData(), primal_offsets);
    primal_dx = 0.0;
...
//    }
    hierarchy_.GetPs(level - 1).Mult(blk_coarse.GetBlock(2), blk_fine.GetBlock(2));
}
